...
static rt_err_t _pipe_check(struct uhintf* intf, upipe_t pipe)
{
    ...
    size = rt_usb_hcd_pipe_xfer(stor->pipe_in->intf->device->hcd, 
    ...
    ustorage_cbw_t cmd, rt_uint8_t* buffer, int timeout)
{
    ...
    {
        /* send the cbw */
        size = rt_usb_hcd_pipe_xfer(stor->pipe_out->intf->device->hcd, stor->pipe_out, 
        ...
        if(cmd->xfer_len != 0)
        {
            pipe = (cmd->dflags == CBWFLAGS_DIR_IN) ? stor->pipe_in :
                stor->pipe_out;
            size = rt_usb_hcd_pipe_xfer(pipe->intf->device->hcd, pipe, (void*)buffer, 
        ...
        /* receive the csw */
        size = rt_usb_hcd_pipe_xfer(stor->pipe_in->intf->device->hcd, stor->pipe_in, 
...
rt_err_t rt_usbh_storage_get_max_lun(struct uhintf* intf, rt_uint8_t* max_lun)
{
    ...
    return rt_usb_hcd_control_xfer(device, &setup, max_lun, 1, timeout);
}
 ...
 * @return the error code, RT_EOK on successfully.
 */
static rt_err_t rt_usbh_storage_enable(struct uhintf* arg)
{
    int i = 0;
    rt_err_t ret;    
    ustor_t stor;
 ...
 * @return the error code, RT_EOK on successfully.
 */
static rt_err_t rt_usbh_storage_disable(struct uhintf* arg)
{
    ustor_t stor;
    struct uhintf* intf = (struct uhintf*)arg;
    ...
    RT_ASSERT(intf->instance != RT_NULL);
    RT_ASSERT(intf->device != RT_NULL);
