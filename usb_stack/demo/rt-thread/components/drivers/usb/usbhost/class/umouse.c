...
static rt_err_t rt_usbh_hid_mouse_callback(struct uhintf* arg)
{
    struct uhid* hid;
#ifdef RT_USING_RTGUI
    ...
    rt_kprintf("HID Mouse: 0x%x 0x%x\n",
                                *(rt_uint32_t*)hid->buffer,
                                *(rt_uint32_t*)(&hid->buffer[4]));
#ifdef RT_USING_RTGUI
    if(hid->buffer[1]!=0)
...
static rt_err_t rt_usbh_hid_mouse_init(struct uhintf* arg)
{
...
/**
 * This function will define the hid mouse protocol, it will be register to the protocol list.
 *
