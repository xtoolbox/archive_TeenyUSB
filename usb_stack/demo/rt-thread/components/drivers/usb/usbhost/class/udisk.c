...
#define UDISK_MAX_COUNT        32
static rt_uint32_t _udisk_idset = 0;
...
static rt_err_t rt_udisk_control(rt_device_t dev, int cmd, void *args)
{
    ...
    data = (struct ustor_data*)dev->user_data;
    stor = (ustor_t)data->intf->instance;
...
#define  UDISK_MOUNT_NAME_FORMAT   "/udisc%d"
...
//        rt_usbh_clear_feature(intf->device, 0, USB_FEATURE_ENDPOINT_HALT);
    ...
    for(i=0; i<MAX_PARTITION_COUNT; i++)
    {
        ...
        if (ret == RT_EOK)
        {
            ...
            rt_snprintf(sname, 8, "sem%d-%d",  data->udisk_id, i);
            ...
            stor->dev_cnt++;
            if (dfs_mount(stor->dev[i].parent.name, mount_name, "elm",
                0, 0) == 0)
            {
        ...
        else
        {
            if(i == 0)
            {
                ...
                stor->dev_cnt++;
                if (dfs_mount(stor->dev[0].parent.name, mount_name,
                    "elm", 0, 0) == 0)
                {
