...
void rt_usbh_root_hub_connect_handler(struct uhcd *hcd, rt_uint8_t port, rt_uint8_t speed)
{
    ...
    hcd->roothub->port_status[port - 1] &= ~PORT_HSDA;
    switch(speed){
...
void rt_usbh_root_hub_enable_handler(struct uhcd *hcd, rt_uint8_t port)
{
    struct uhost_msg msg;
    msg.type = USB_MSG_CONNECT_CHANGE;
    msg.content.hub = hcd->roothub;
    if(hcd->roothub->port_status[port - 1] & PORT_PES){
    }else{
      hcd->roothub->port_status[port - 1] |= PORT_CCS | PORT_CCSC | PORT_PES | PORT_PESC;
      rt_usbh_event_signal(&msg);
    }
}
...
void rt_usbh_root_hub_disconnect_handler(struct uhcd *hcd, rt_uint8_t port)
{
    ...
    msg.type = USB_MSG_CONNECT_CHANGE;
    msg.content.hub = hcd->roothub;
    hcd->roothub->port_status[port - 1] = 0;
    hcd->roothub->port_status[port - 1] |= PORT_CCSC;
    //hcd->roothub->port_status[port - 1] &= ~PORT_CCS;
    //hcd->roothub->port_status[port - 1] &= ~PORT_PES;
    rt_usbh_event_signal(&msg);
}
...
rt_err_t rt_usbh_hub_get_status(struct uinstance* device, rt_uint32_t* buffer)
{
    ...
    setup.wLength = 4;
    setup.wValue = 0;
    return rt_usb_hcd_control_xfer(device, &setup, buffer, 4, timeout);
}
...
rt_err_t rt_usbh_hub_get_port_status(uhub_t hub, rt_uint16_t port, rt_uint32_t* buffer)
{
    ...
    setup.wLength = 4;
    setup.wValue = 0;
    return rt_usb_hcd_control_xfer(hub->self, &setup, buffer, 4, timeout);
}
...
rt_err_t rt_usbh_hub_clear_port_feature(uhub_t hub, rt_uint16_t port, rt_uint16_t feature)
{
    ...
    return rt_usb_hcd_control_xfer(hub->self, &setup, RT_NULL, 0, timeout);
}
...
static rt_err_t rt_usbh_hub_port_change(uhub_t hub)
{
    ...
    for (i = 0; i < hub->num_ports; i++)
    {
        ...
        if (pstatus & PORT_PRSC){
          rt_usbh_hub_clear_port_feature(hub, i + 1, PORT_FEAT_C_RESET);
        }
        ...
        {            
            ...
            device->speed = PORT_SPEED_FULL;
            if(pstatus & PORT_LSDA)
              device->speed = PORT_SPEED_LOW;
            if(pstatus & PORT_HSDA)
              device->speed = PORT_SPEED_HIGH;
 ...
 * @return none.
 */
static void rt_usbh_hub_irq(upipe_t context)
{
    upipe_t pipe;
    uhub_t hub;
    int timeout = USB_TIMEOUT_BASIC;
...
static rt_err_t rt_usbh_hub_enable(struct uhintf *arg)
{
    int i = 0;
    rt_err_t ret = RT_EOK;
 ...
 * @return the error code, RT_EOK on successfully.
 */
static rt_err_t rt_usbh_hub_disable(struct uhintf* arg)
{
    int i;
    uhub_t hub;
    struct uhintf* intf = (struct uhintf*)arg;
...
void rt_usbh_hub_init(uhcd_t hcd)
{
    ...
    uhub_t root_hub;
    root_hub = (uhub_t)rt_malloc(sizeof(struct uhub));
    if(!root_hub){
      rt_kprintf("Fail to allocate memory for root HUB");
      return;
    }
    rt_memset((void *)root_hub, 0, sizeof(struct uhub));
...
{
  uhub_t root_hub = hcd->roothub;
  rt_uint8_t i;
  for(i=0;i<root_hub->num_ports;i++){
    if(root_hub->child[i]){
      rt_usbh_detach_instance(root_hub->child[i]);
    }
  }
  rt_free(root_hub);
}
