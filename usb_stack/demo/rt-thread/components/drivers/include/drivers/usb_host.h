...
struct uclass_driver
{
    ...
    rt_err_t (*enable)(struct uhintf* arg);
    rt_err_t (*disable)(struct uhintf* arg);
};
...
struct uprotocal
{
    ...
    rt_err_t (*init)(struct uhintf* arg);
    rt_err_t (*callback)(struct uhintf* arg);    
};
...
struct uhintf
{
    ...
    ucd_t drv;
    void* instance;
    uint8_t  index;   // this == device->intf[this->idx]
};
...
struct upipe
{
    ...
    void* periodic_buffer;
    rt_uint32_t  periodic_size;
    void(*callback)(struct upipe*);
    void* backend;
};
...
struct uhcd_ops
{
    rt_err_t    (*reset_port)   (struct uhcd* hcd, rt_uint8_t port);
...
rt_err_t rt_usbh_class_driver_enable(ucd_t drv, struct uhintf* args);
rt_err_t rt_usbh_class_driver_disable(ucd_t drv, struct uhintf* args);
...
rt_inline rt_err_t rt_usb_hcd_alloc_pipe(uhcd_t hcd, upipe_t* pipe, struct uhintf* intf, uep_desc_t ep)
{
...
rt_inline void rt_usb_pipe_add_callback(upipe_t pipe, void( *callback)(struct upipe*))
{
